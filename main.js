...
(function() {
  ...
  // Position the camera to frame the square room.  The lookAt
  // coordinates target roughly the centre of the floor (half of
  // floorWidth and floorDepth).  This provides a pleasing angle
  // reminiscent of the reference scene.
  camera.position.set(10, 8, 10);
  camera.lookAt(4, 1.8, 4);
  ...
  function buildRoom() {
    // Dimensions
    // Room footprint: make both width and depth equal so the base is a
    // square.  This better matches the proportions of the original
    // mini room.
    const floorWidth = 8;
    const floorDepth = 8;
  ...
  // Position: close to the left wall and centred along depth.  Because the
  // room is now square (8×8), place the sofa at x≈1.5 and z≈4.
  sofa.position.set(1.5, 0, 4);
  scene.add(sofa);
  ...
  // Position the table directly in front of the sofa, slightly farther
  // into the room.  Align the z position with the sofa (≈4) and set x≈3.5.
  table.position.set(3.5, 0, 4);
  scene.add(table);
  // Rug beneath sofa and table; scaled to fit the new layout
  const rug = createRug();
  rug.scale.set(0.8, 1.0, 0.8);
  // Place the rug under the sofa and table.  Centre at x≈3 and z≈4.
  rug.position.set(3.0, 0.0, 4);
  scene.add(rug);
  // Plant in the back corner near the sofa
  const plant = createPlant();
  plant.scale.set(0.8, 0.8, 0.8);
  // Move the plant towards the front‑left corner.  For an 8×8 room
  // the front edge is at z≈8; place the plant at z≈6.5.
  plant.position.set(0.8, 0, 6.5);
  scene.add(plant);
  // Clothing rack behind the sofa along the back wall
  const rack = createClothingRack();
  rack.scale.set(0.8, 0.8, 0.8);
  // Position the clothing rack along the back wall.  Keep a small
  // offset from the left wall (x≈2.5) and from the wall thickness (z≈1.0).
  rack.position.set(2.5, 0, 1.0);
  // Rotate the rack so the shirts face into the room
  ...
  // Place the fridge near the right wall.  The room width is 8, so set
  // x≈7.6 (accounting for half the fridge width) and z≈1.5.
  fridge.position.set(7.6, 1.0, 1.5);
  scene.add(fridge);
  // Coffee machine on top of fridge
  const coffeeMachine = createCoffeeMachine();
  coffeeMachine.scale.set(0.5, 0.5, 0.5);
  coffeeMachine.position.set(7.6, 2.2, 1.5);
  scene.add(coffeeMachine);
  // Desk with laptop near fridge on the right wall
  const desk = createDesk();
  desk.scale.set(0.8, 0.8, 0.8);
  // Place the desk against the right wall, centred along depth.  Use
  // x≈7.5 and z≈4.  Rotate so the desk faces left.
  desk.position.set(7.5, 0, 4);
  ...
  // Mount the window above the desk on the right wall.  Use x≈7.97
  // (close to the wall), and align vertically at y≈1.8.  Set z≈4 to
  // centre the window over the desk.
  houseWindow.position.set(7.97, 1.8, 4);
  houseWindow.rotation.y = -Math.PI / 2;
  scene.add(houseWindow);
  const curtains = createCurtains();
  curtains.position.set(7.99, 1.8, 4);
  ...
  // Place the robot vacuum near the centre of the room towards the front.
  robotVacuum.position.set(4.0, 0, 3.5);
  scene.add(robotVacuum);
  // Cat outside the window looking in
  const cat = createCat();
  cat.scale.set(0.7, 0.7, 0.7);
  // Place the cat outside the window.  Position slightly beyond the
  // right wall (x≈8.3) and align with the window’s centre (z≈4).
  cat.position.set(8.3, 0, 4);
  cat.rotation.y = Math.PI / 2;
  scene.add(cat);
  // Dog sitting on the sofa
  const dog = createDog();
  dog.scale.set(0.7, 0.7, 0.7);
  // Seat the dog on the sofa.  Adjust z to align with the sofa’s
  // centre (≈4).
  dog.position.set(1.8, 0.5, 4);
  ...
  // Position picture frames evenly across the back wall.  Use three
  // equally spaced x positions and mount them slightly above the sofa.
  addPictureFrame('assets/logo21.jpg', 0.9, 0.9, 0.08, { x: 2.0, y: 2.4, z: 0.08 }, 0);
  addPictureFrame('assets/tattoo.jpg', 0.9, 0.9, 0.08, { x: 4.0, y: 2.4, z: 0.08 }, 0);
  addPictureFrame('assets/cat_photo.jpg', 0.9, 0.9, 0.08, { x: 6.0, y: 2.4, z: 0.08 }, 0);
  // Right wall images (facing –x).  Place above the desk and fridge.  Set
  // x near the right wall (≈7.85) and choose z positions corresponding
  // to the desk (≈4) and fridge (≈1.5).
  addPictureFrame('assets/friend_top.jpg', 0.8, 1.0, 0.08, { x: 7.85, y: 2.4, z: 4.5 }, -Math.PI / 2);
  addPictureFrame('assets/ready_player_one.jpg', 0.8, 1.2, 0.08, { x: 7.85, y: 1.8, z: 2.0 }, -Math.PI / 2);
